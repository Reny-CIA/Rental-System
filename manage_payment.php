...
// Check if tenant has any previous payments
$is_new_tenant = false;
$tenant_id = isset($tenant_id) ? $tenant_id : 0; // Prevent undefined var warning
if ($tenant_id) {
    $check = $conn->query("SELECT COUNT(*) as cnt FROM payments WHERE tenant_id = $tenant_id");
...
// Function to avoid negative balance
function safeBalance($amount) {
...
<div class="container-fluid">
    ...
    <form action="" id="manage-payment">
        ...
        <?php if ($is_new_tenant): ?>
            <div class="form-group">
                <label>House Deposit</label>
                <input type="number" class="form-control" name="house_deposit" step="any" value="0">
            </div>
            <div class="form-group">
                <label>Water Deposit</label>
                <input type="number" class="form-control" name="water_deposit" step="any" value="0">
            </div>
            <div class="form-group">
                <label>Electricity Deposit</label>
                <input type="number" class="form-control" name="electricity_deposit" step="any" value="0">
            </div>
        <?php else: ?>
            <div class="form-group">
                <label>Water Bill</label>
                <input type="number" class="form-control" name="water_bill" step="any" value="0">
            </div>
        <?php endif; ?>
